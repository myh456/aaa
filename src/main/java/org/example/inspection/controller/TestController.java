...
public class TestController {
        ...
        try {
            ...
            for (int i = 0; i < excelConfigs.size(); i++) {
                ...
                if (tableObj instanceof Map) {
                    ...
                    if (headerMap != null) {
                        // 处理表头（包括标题行和多级表头）
                        currentRow = processTableHeaders(writer, title, headerConfig, headerMap, currentRow);
                        ...
                        if (tableData instanceof List) {
                            ...
                            // 使用passRows确保数据写入到正确的位置
                            writer.passRows(currentRow);
                            writer.write(emptyList, false);
                            currentRow += 1; // 只增加一行（表头行）
                        }
                    }
                    ...
                    for (Map<String, Object> table : tableMaps) {
                        ...
                        if (headerMap != null) {
                            // 处理表头（包括标题行和多级表头）
                            currentRow = processTableHeaders(writer, title, headerConfig, headerMap, currentRow);
                            // 计算当前表格的表头行数（包括标题行和表头行）
                            int tableHeaderRows = getTableHeaderRows(title, headerConfig, headerMap);
                            // 数据在json中取出
    ...
    /**
     * 处理Excel表头，包括标题行和多级表头
     *
     * @param writer       Excel写入器
     * @param title        表格标题
     * @param headerConfig 表头配置对象
    ...
    private static int processTableHeaders(ExcelWriter writer, String title, ExcelHeaderConfig headerConfig, Map<String, String> headerMap, int currentRow) {
        ...
        if (headerConfig != null) {
            org.example.inspection.utils.ExcelUtil.createMultiLevelHeader(writer, headerConfig, headerMap, currentRow);
            currentRow += headerConfig.getHeaderRowCount(); // 更新当前行位置
        }
        return currentRow;
    }
    ...
    private static int getTableHeaderRows(String title, ExcelHeaderConfig headerConfig, Map<String, String> headerMap) {
        ...
        if (headerConfig != null) {
            tableHeaderRows += headerConfig.getHeaderRowCount(); // 表头行
        } else if (headerMap != null) {
            tableHeaderRows++; // 默认表头行
        }
